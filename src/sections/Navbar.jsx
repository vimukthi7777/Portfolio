...
const Navbar = () => {
    ...
    useGSAP(() => {
        ...
        tl.current = gsap.timeline({paused:true}).to(navRef.current, {
            ...
            linksRef.current,
            {
                autoAlpha:1,
                x:0,
                stagger: 0.1, //0.1s after previous one
                duration:0.5,
                ease: "power2.out",
            },
            "<" //start animation at the same time with first one
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            setShowBurger(currentScrollY <= lastScrollY ||
            currentScrollY < 10);
  ...
  return (
    ...
    rounded-full cursor-pointer w-14 h-14 md:w-20 md:h-20 top-4 right-10' onClick={toggleMenu} 
    style={showBurger
